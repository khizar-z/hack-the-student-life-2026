...
## 9. Known Constraints and Honest Demo Framing
Use this language if asked about limitations:

1. Dataset scope
- "For demo reliability, we currently use a curated set of 85 UofT CS faculty profiles."

2. Data quality variability
- "Some fields from external sources can be sparse or noisy, so we use graceful fallbacks and still preserve usability."

3. Image availability
- "Profile photos are best-effort from public profile metadata; otherwise we show a consistent fallback avatar."

4. Reliability without cloud credentials
- "If AWS credentials are unavailable, search still works via keyword fallback, and AI outputs use predefined fallbacks where needed."

That framing scores better than pretending everything is perfect.

---

## 10. Demo Plan (3-Min Presentation + 3-Min Q&A)

## 10.1 3-minute presentation script

### 0:00-0:20 Problem and challenge fit
Say:
- "Students struggle to discover professors and research opportunities because information is fragmented."
- "Panda solves Challenge Area 3 by improving discovery, visibility, and personalized matching across UofT." 

### 0:20-0:45 Landing and search
Do:
- Show landing page
- Type a query (example: "machine learning for healthcare")

Say:
- "Search is semantic using AWS Titan embeddings, so we match by meaning rather than exact keyword overlap."

### 0:45-1:15 Filter and browse
Do:
- Show subject filter on sidebar
- Click "Browse" to show default alphabetical mode

Say:
- "Students can either explore broadly with Browse or narrow by subject filters."

### 1:15-1:55 Professor detail experience
Do:
- Open one professor card
- Show generated profile description
- Show right-side paper list

Say:
- "Each profile gives a quick AI-generated overview and direct paper context so students can assess fit quickly."

### 1:55-2:30 Paper intelligence
Do:
- Click a paper
- Show abstract + key takeaways popup

Say:
- "Instead of reading every abstract manually, students get key takeaways instantly, reducing research onboarding time."

### 2:30-2:55 Reach-out guidance
Do:
- Click Reach out
- Upload resume (or use background text)
- Generate pointers

Say:
- "Panda turns discovery into action by generating concrete outreach pointers grounded in both student profile and professor context."

### 2:55-3:00 Close
Say:
- "Panda is a practical AWS-powered workflow from discovery to outreach, designed for immediate deployment as a campus proof-of-concept."

---

## 10.2 Q&A strategy (3 minutes)
Have one owner per topic:
- Person A: user/problem fit + challenge alignment
- Person B: architecture and retrieval pipeline
- Person C: AWS services, model/fallback decisions
- Person D: product impact, rollout feasibility

Likely questions and strong answers:

1. "Why is this technically sophisticated?"
- "We combine semantic retrieval, generative personalization, profile enrichment, and graceful degradation in one integrated workflow."

2. "What happens when AWS is unavailable?"
- "Search falls back to keyword ranking; summaries and takeaways have deterministic fallbacks, so core demo flow remains operational."

3. "How is this feasible to deploy?"
- "It is a lightweight FastAPI + static frontend architecture using managed AWS services, so it can be deployed quickly and scaled incrementally."

4. "How does this improve student life concretely?"
- "It reduces discovery time, improves quality of outreach, and makes research opportunities easier to access for more students."

---

## 11. Rubric-Specific Talking Points (Use These Exact Angles)

### Innovation and Relevance (10)
Say:
- "We did not build just a directory or just a chatbot; we built an end-to-end discovery-to-outreach workflow tailored to UofT research matching."

### Technical Complexity (15)
Say:
- "Our system combines scraping, third-party enrichment, vector search, LLM generation, caching, analytics logging, and resilient fallback logic."

### Working Demo (10)
Say:
- "The full flow is live: search -> filter -> profile -> papers -> takeaways -> outreach pointers."

### Presentation (5)
Say:
- "We will show one user journey with clear technical narration at each step."

### Feasibility and Effectiveness (10)
Say:
- "Built on managed AWS services and simple app architecture, this is realistic to pilot as a departmental tool with low operational overhead."

---

## 12. Technical Deep-Dive Notes for Team Members

If judges ask for implementation specifics:
- Retrieval math:
  - embeddings matrix normalized at startup
  - cosine similarity via matrix-vector multiply
- Performance:
  - in-memory vector matrix (small dataset) gives fast scoring
  - generated summaries/descriptions are cached
- Robustness:
  - API endpoints validate/limit inputs
  - fallback search and fallback text generation paths
- Security considerations for future work:
  - add auth and per-user data boundaries
  - harden file upload limits and scanning
  - stricter prompt injection controls for generative endpoints
